...
#define ADDR_JP1PORT ((volatile char *) 0x10000060)
...
// Green LEDs above KEY3 flash when KEY3 is pressed
void flashGreenLEDs() {
	*GLEDs = 0xc0;
...
void flashRedLEDLine() {
	...
	while (bitsToWrite != 0) {
		...
		delay = 0;
		bitsToWrite >>= 1;	// Shift bits right by one bit
	}
	
	*RLEDs = 0x0000;
}

// Prints each character as the user types it
...
void printMessage (unsigned input) {
	...
	if (input == 0xfa) {
		...
		if (input == 0x5A && saveNextChar == 0) {
			...
			flashRedLEDs(); // Make the red LEDs flash altogether
			
			// If message received while nothing has been typed
			// clear >>: from line
			if (mts_index == 0) {
				printf("%c[2K", 27);		// Clear terminal line
				printf("%c[%dD", 27, 4);	// Move the terminal cursor back 4 columns
				printf("<<: ");				// New prompt
				...
				// Print out the not sent terminal input
...
// Clears all text on the terminal screen
void eraseTerminal() {
	printf("%c[2J", 27);
	...
	*ADDR_JP1PORT = 0xfa;	// Write 0xfa to JP1 to notify other user that the terminal has been erased (clear received message on other computer)
}